...
std::vector<T> normalize(
  ...
  for (size_t b = 0; b < batchSz; ++b) {
    ...
    T sum = std::accumulate(start, start + inSizes[b], 0.0);
    T mean = sum / float(inSizes[b]);
    std::transform(
        start, start + inSizes[b], start, [mean](T x) { return x - mean; });
    T sq_sum = std::inner_product(start, start + inSizes[b], start, 0.0);
    T stddev = std::sqrt(sq_sum / float(inSizes[b]));
    if (stddev > threshold) {
      std::transform(start, start + inSizes[b], start, [stddev](T x) {
        return x / stddev;
      });
    }
  }
